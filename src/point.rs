...
impl Point {
  ...
  pub fn get_points_around_indirect(&self) -> [Option<Point>; 8] {
    [
      Some(*self + X),
      Some(*self + Y),
      Some(*self + Y + X),
      self.checked_sub(&X),
      self.checked_sub(&Y),
      self.checked_sub(&Y).and_then(|v| v.checked_sub(&X)),
      Some(*self + X).and_then(|v| v.checked_sub(&Y)),
      Some(*self + Y).and_then(|v| v.checked_sub(&X)),
    ]
  }
